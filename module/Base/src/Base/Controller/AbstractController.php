        ...
        if ($request->isPost()){
            ...
            if ($form->isValid()){
                ...
                if ($service->save($datas)){
                    // Inicio Geração de Log de Movimentação de Inserção no Banco
                    /*
                    $serviceHistorico = $this->getServiceLocator()->get('Admin\Service\HistoricosService');
                    // Id do Ultimo Registro Cadastro
                    $idRegistro = $this->getEm()->getRepository($this->entity)->UltimoRegistro();

                    //var_dump($idRegistro[1]);die();
                    $historico['evento'] = 'Cadastro';
                    $historico['evento_id'] = $idRegistro[1];
                    $historico['usuario'] = $this->getDados()['login']->getId();
                    $historico['admin'] = 1;
                    $historico['tipo'] = 1;
                    $historico['entidade'] = $this->entity;
                    $historico['movimentacao'] = 'Cadastro de um(a) ' . $this->controller;
                    if($this->enviar == 1):
                        $enviaEmail = new EnviarEmail($this->rementente,$datas['email'],$this->titulo,$this->assunto);
                        $enviaEmail->enviar();
                    endif;
                    $serviceHistorico->save($historico);
                    // Fim Geração de Log de Movimentação de Inserção no Banco
                    */
